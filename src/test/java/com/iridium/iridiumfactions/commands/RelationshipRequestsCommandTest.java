...
public class RelationshipRequestsCommandTest {
    ...
    @Test
    public void executeRelationshipRequestsCommandNoFaction() {
        PlayerMock playerMock = new UserBuilder(serverMock).build();

        serverMock.dispatchCommand(playerMock, "f requests");
    ...
    @Test
    public void executeRelationshipRequestsCommandSuccessful() {
        Faction faction = new FactionBuilder().build();
        PlayerMock playerMock = new UserBuilder(serverMock).withFaction(faction).build();

        serverMock.dispatchCommand(playerMock, "f requests");

        assertTrue(playerMock.getOpenInventory().getTopInventory().getHolder() instanceof FactionRelationshipRequestsGUI);
        playerMock.assertNoMoreSaid();
    }
}