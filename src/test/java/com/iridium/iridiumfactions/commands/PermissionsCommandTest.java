...
class PermissionsCommandTest {
    ...
    @Test
    public void executePermissionsCommandNoFaction() {
        PlayerMock playerMock = new UserBuilder(serverMock).build();

        serverMock.dispatchCommand(playerMock, "f permissions");
    ...
    @Test
    public void executePermissionsCommandSuccessful() {
        Faction faction = new FactionBuilder().build();
        PlayerMock playerMock = new UserBuilder(serverMock).withFaction(faction).build();

        serverMock.dispatchCommand(playerMock, "f permissions");
        assertTrue(playerMock.getOpenInventory().getTopInventory().getHolder() instanceof FactionRanksGUI);
    }
