...
public class LocationUtils {
    ...
    public static boolean isSafe(Location location) {
        if (IridiumFactions.getInstance().isTesting()) return true;
        Block block = location.getBlock();
        Block above = location.clone().add(0, 1, 0).getBlock();
        Block below = location.clone().subtract(0, 1, 0).getBlock();
        MultiVersion multiVersion = IridiumFactions.getInstance().getMultiVersion();
    ...
    /**
     * Gets a safe location on the Faction
     *
     * @param location The location we want to teleport
     * @param faction   The faction we are inside
     * @return A safe Location, if none found return null
     */
    @Nullable
    public static Location getSafeLocation(Location location, Faction faction) {
        //TODO
//        World world = location.getWorld();
//        if (world == null) return location;
//        if (faction == null) return location;
...
//
//        Location pos1 = faction.getPosition1(world);
//        Location pos2 = faction.getPosition2(world);
...
//        }
    ...
    private static Location getHighestLocation(int x, int z, World world) {
        ...
        while (!IridiumFactions.getInstance().getMultiVersion().isPassable(block)) {
