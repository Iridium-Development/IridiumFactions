...
public class UnClaimCommand extends Command {
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        if (args.length == 1) {
            IridiumFactions.getInstance().getFactionManager().unClaimFactionLand(faction.get(), player.getLocation().getChunk(), player);
            return true;
        }
        try {
            int radius = Integer.parseInt(args[1]);
            IridiumFactions.getInstance().getFactionManager().unClaimFactionLand(faction.get(), player.getLocation().getChunk(), radius, player);
            return true;
