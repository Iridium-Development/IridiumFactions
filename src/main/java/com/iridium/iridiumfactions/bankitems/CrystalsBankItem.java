package com.iridium.iridiumfactions.bankitems;
...
public class CrystalsBankItem extends BankItem {
    ...
    public BankResponse withdraw(Player player, Number amount, TeamBank teamBank, IridiumTeams<?, ?> iridiumTeams) {
        ...
        if (crystals > 0) {
            player.getInventory().addItem(IridiumFactions.getInstance().getFactionManager().getFactionCrystal(crystals)).values().forEach(itemStack ->
    ...
    private int removeCrystals(Inventory inventory) {
        ...
        for (int slot = 0; slot < inventory.getContents().length; slot++) {
            ...
            int crystalsPerItem = IridiumFactions.getInstance().getFactionManager().getFactionCrystals(itemStack);
